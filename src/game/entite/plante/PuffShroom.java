...
public class PuffShroom implements PlanteInterface {
	private int x;
	private int y;
	private Color color;
	private int size;
	...
	private PuffShroom(int x, int y, int health, int damage, int size, Color color) {
		this.x = x;
		this.y = y;
		this.color = color;
		this.health = health;
	...
	public static PuffShroom SpawnPuffShroom(int x, int y) {
		return new PuffShroom (x, y, 300, 20, 50, new Color(13, 234, 147));
	}

	@Override
	public String toString() {
		return "Puffshroom [health=" + health + " coordonne : " + x + "   " + y + "]";
	}
	...
	public PlanteInterface haveSame() {
		return Seashroom.SpawnPeashooter(x, y);
	}
