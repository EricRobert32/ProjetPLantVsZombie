...
public class Peashooter implements PlanteInterface {
	private int x;
	private int y;
	private Color color;
	private int size;
	...
	private Peashooter(int x, int y, int health, int damage, int size, Color color) {
		this.x = x;
		this.y = y;
		this.color = color;
		this.health = health;
	...
	public static Peashooter SpawnPeashooter(int x, int y) {
		return new Peashooter(x, y, 300, 20,150, new Color(0, 251, 13));
	}

	@Override
	public String toString() {
		return "Peashooter [health=" + health + " coordonne : " + x + "   " + y + "]";
	}
